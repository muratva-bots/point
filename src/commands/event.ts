...
const Command: Point.ICommand = {
    ...
    execute: async ({ client, message, args, guildData }) => {
        ...
        if (ms(timing) > ms("3d") || ms(timing) < ms("1m"))
            ...
            message,"Maksimum 3 gün minimum 1 dakikalık etkinlik başlatabilirsiniz!",
				);

        const timestamp = Date.now() + ms(timing);
        message.reply(`Etkinlik başlatıldı. (${time(Math.floor(timestamp / 1000), 'R')}) boyunca yetkililer 2 katı puan kazanacak.`);
